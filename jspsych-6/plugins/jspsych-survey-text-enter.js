 ...
 *
 * edited by Becky Gilbert to allow users to end trial with an Enter key press
 *
 */


jsPsych.plugins['survey-text-enter'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'survey-text-enter',
    description: '',
    parameters: {
      questions: {
      ...
      allow_enter: {
        ...
        pretty_name: 'Allow enter',
        default: false,
        description: 'Whether or not to end trial with an Enter key press'
      }
    }
  };
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].rows == 1){
        html += '<input type="text" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'" autocomplete="off"></input>';
      } else {
        html += '<textarea name="#jspsych-survey-text-response-' + i + '" cols="' + trial.questions[i].columns + '" rows="' + trial.questions[i].rows + '" autocomplete="off">'+trial.questions[i].value+'</textarea>';
      }
      html += '</div>';
    }
    ...
    if (trial.allow_enter) {
      var keyboard_listener = jsPsych.pluginAPI.getKeyboardResponse({
        callback_function: function() {display_element.querySelector('#jspsych-survey-text-next').click();},
